...
check("cancel/speak 竞态修复（延迟 + 代际）", "speechGen" in HTML and "setTimeout" in HTML)
check("utterance 防 GC 引用", "utteranceRefs" in HTML)
check("刷新后 resume 推动合成器", "speechSynthesis.resume" in HTML)
check("首次交互补读兜底", "pointerdown" in HTML)
check("启动门（手势解锁语音）", 'id="start-gate"' in HTML and "initStartGate" in HTML)
check("风声 BGM（且听风吟场景）", 'assets/风声.mp3' in HTML and "windAudio" in HTML and "isSpecialListen" in HTML)
check("预渲染语音包（mp3 双轨）", 'assets/audio/manifest.json' in HTML and "playSceneAudio" in HTML and "usedAI" in HTML)
check("语速滑块 + 试听", 'id="sp-rate"' in HTML and 'id="sp-preview"' in HTML)
check("角色差分基于基础语速（系数制）", "speechPref.rate" in HTML)
check("多音字修正表钩子", "PRONUNCIATION_FIXES" in HTML)
